    ...
    if request.method == "POST" and request.FILES['the_files']:
        ...
        for file in os.listdir(str(BASE_DIR)+'/temp_dir'):
            ...
            if request.POST['position'] == 'tl':
                # Top left
                c.drawImage(str(BASE_DIR) + '/media/application_images/stamp.png', 0, int(input_file.pages[0].mediabox.height) - 100, 130, 130 * int(img_height) / int(img_width), mask='auto')

            if request.POST['position'] == 'tr':
                # Top right
                c.drawImage(str(BASE_DIR) + '/media/application_images/stamp.png', int(input_file.pages[0].mediabox.width) - 150, int(input_file.pages[0].mediabox.height) - 100, 130, 130 * int(img_height) / int(img_width), mask='auto')

            if request.POST['position'] == 'br':
                # Bottom right
                c.drawImage(str(BASE_DIR) + '/media/application_images/stamp.png', int(input_file.pages[0].mediabox.width) - 150, 0, 130, 130 * int(img_height) / int(img_width), mask='auto')
